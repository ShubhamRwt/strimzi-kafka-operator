...
This feature will allow you to move the data between the JBOD disks using the `KafkaRebalance` custom resource that we have in Strimzi. This feature makes use of the `remove-disks` endpoint of Cruise Control that triggers a rebalancing operation which moves replicas, starting with the largest and proceeding to the smallest, to the remaining disks. 
...
We hope this blog post has provided you with a clear understanding of how you can use the `KafkaRebalance` custom reosurce in `remove-disks` to easily move the data between the JBOD disks. 
